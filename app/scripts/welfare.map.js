...
welfare.map = function () {
  ...
  var setupMap = function setupMap(layerObj) {
    map = new google.maps.Map(domMap.canvas, configMap.mapOptions);
    Object.keys(layerObj).forEach(function (key) {
      var obj = {};
  ...
  var loadMap = function loadMap(active, previous) {
    ...
    if (active === previous && active !== undefined && layers[active].visible || previous && previous !== '/' && (active === '/' || !active)) {
    ...
    var data = welfare.shell.getLayerData(active);
    if (!data) {
      var promise = new Promise(function (resolve, reject) {
        try {
          layers[active].dataLayer.loadGeoJson(layers[active].url, {}, function (features) {
            resolve(features);
          });
        } catch (err) {
          reject(err);
    ...
    if (!layers[active].eventAdded) {
      ...
      layers[active].dataLayer.addListener('click', function (event) {
        // Create custom event and pass the clicked layer data to it
        var projectUpdateEvent = new CustomEvent('updateProject', {
          detail: {
            project: event
          }
        });
  ...
  var getDataLayer = function getDataLayer() {
    return layers[stateMap.activeLayer].dataLayer;
  };

  var isLayerVisible = function isLayerVisible(id) {
    return layers[id].visible;
  };

  var updateMapState = function updateMapState(event) {
  ...
  var onUpdateProject = function onUpdateProject(event) {
    var feature = event.detail.project.feature;
    var dataLayer = layers[stateMap.activeLayer].dataLayer;
  ...
  var initModule = function initModule(layerObj) {
    setDOMmap();
    setupMap(layerObj);
